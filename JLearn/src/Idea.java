...
public class Idea {
    ...
    */
    ...
    public static void scanBoard() {
        if (JLearn.board[0][0] == 4) {
            JLearn.Win(3);
            System.exit(0);
        }
        else if (JLearn.board[0][0] == 5) {
            JLearn.Win(5);
            System.exit(0);
        }
        ...
        for(i = 1;i <= 19;i++){
        	for(j = 1;j <= 19;j++){
        		if (JLearn.board[i][j] == 1||JLearn.board[i][j] == 2) {
                    fillMove(i, j, JLearn.board[i][j]);
                }
        	}
        }
    }
                	...
                	for(k = 1;k < 9;k++){
                		switch (move[i][j][k]) {
               			...
               			case 3: {
                        	switch (k) {
                            case 1: {
                                if (JLearn.board[i + 1][j + 1] == colors[i][j][k]) {
                                ...
                                else if (JLearn.board[i - 4][j - 4] == 0 || JLearn.board[i + 1][j + 1] == 0) { 
                                	if (JLearn.board[i - 4][j - 4] == 0 && JLearn.board[i + 1][j + 1] == 0) {
                            ...
                            case 2: {
                                if (JLearn.board[i][j + 1] == colors[i][j][k]) {
                                	move[i][j][0] += moves[1];//moves[1]_2
                                    if (colors[i][j][k] == JLearn.You) {
                                    	move[i][j][0] += moves[1];//I-priority
                                    }
                                ...
                                else if (JLearn.board[i][j - 4] == 0 || JLearn.board[i][j + 1] == 0) {
                                    if (JLearn.board[i][j - 4] == 0 && JLearn.board[i][j + 1] == 0) {
                                    	move[i][j][0] += moves[3];//move_3_1_open
                                		if (colors[i][j][k] == JLearn.You) {
                                        	move[i][j][0] += moves[3];//I-priority
                                        }
                            ...
                            case 4: {
                                if (JLearn.board[i + 1][j] == colors[i][j][k]) {
                                	move[i][j][0] += moves[1];//moves[1]_2
                                    if (colors[i][j][k] == JLearn.You) {
                                    	move[i][j][0] += moves[1];//I-priority
                                    }
                                }
                                else if (JLearn.board[i - 4][j] == 0 || JLearn.board[i + 1][j] == 0) {
                                    if (JLearn.board[i - 4][j] == 0 && JLearn.board[i + 1][j] == 0) {
                                    	move[i][j][0] += moves[3];//move_3_1_open
                                		if (colors[i][j][k] == JLearn.You) {
                                        	move[i][j][0] += moves[3];//I-priority
                                        }
                            ...
                            case 1: {
                            	if (JLearn.board[i + 1][j + 1] == colors[i][j][k]) {
                            		if(move[i][j][8] == 2){
                            			...
                            			if (JLearn.board[i + 2][j + 2] == 0 && JLearn.board[i - 3][j - 3] == 0){
                            				move[i][j][0] += moves[5];//move_3_2_open
                                    		if (colors[i][j][k] == JLearn.You) {
                                            	move[i][j][0] += moves[5];//I-priority
                                            }
                            			}
                            			else if(JLearn.board[i + 2][j + 2] == 0 || JLearn.board[i - 3][j - 3] == 0){
                            				move[i][j][0] += moves[6];
                            				multi3[i][j][k]=1;
                                    	}
                            		}
                            	}
                            	else if(JLearn.board[i + 1][j + 1] == 0 || JLearn.board[i - 3][j - 3] == 0){
                            		if(JLearn.board[i + 1][j + 1] == 0 && JLearn.board[i - 3][j - 3] == 0){
                            			move[i][j][0] += moves[7];
                            			multi3[i][j][k]=1;
                            		}
                            		else{
                            ...
                            case 2: {
                            	if (JLearn.board[i][j + 1] == colors[i][j][k]) {
                            		if(move[i][j][7] == 2){
                            			...
                            			if (JLearn.board[i][j + 2] == 0 && JLearn.board[i][j - 3] == 0){
                            				move[i][j][0] += moves[5];//move_3_2_open
                                    		if (colors[i][j][k] == JLearn.You) {
                                            	move[i][j][0] += moves[5];//I-priority
                                            }
                            			}
                            			else if(JLearn.board[i][j + 2] == 0 || JLearn.board[i][j - 3] == 0){
                            				move[i][j][0] += moves[6];
                            				multi3[i][j][k]=1;
                            			}
                            		}
                            	}
                            	else if(JLearn.board[i][j + 1] == 0 || JLearn.board[i][j - 3] == 0){
                            		if(JLearn.board[i][j + 1] == 0 && JLearn.board[i][j - 3] == 0){
                            			move[i][j][0] += moves[7];
                            			multi3[i][j][k]=1;
                            		}
                            		else{
                            ...
                            case 4: {
                            	if (JLearn.board[i + 1][j] == colors[i][j][k]) {
                            		if(move[i][j][5] == 2){
                            			...
                            			if (JLearn.board[i + 2][j] == 0 && JLearn.board[i - 3][j] == 0){
                            				move[i][j][0] += moves[5];//move_3_2_open
                                    		if (colors[i][j][k] == JLearn.You) {
                                            	move[i][j][0] += moves[5];//I-priority
                                            }
                            			}
                            			else if(JLearn.board[i + 2][j] == 0 || JLearn.board[i - 3][j] == 0){
                            				move[i][j][0] += moves[6];
                            				multi3[i][j][k]=1;
                            			}
                            		}
                            	}
                            	else if(JLearn.board[i + 1][j] == 0 || JLearn.board[i - 3][j] == 0){
                            		if(JLearn.board[i + 1][j] == 0 && JLearn.board[i - 3][j] == 0){
                            			move[i][j][0] += moves[7];
                            			multi3[i][j][k]=1;
                            		}
                            		else{
    ...
    public static void aIntelligence() {
        ...
        JLearn.board[x][y] = JLearn.You;
        
        for(i = 1;i <= 19;i++){
        	for(j = 1;j <= 19;j++){
        		System.out.print(JLearn.board[j][i]+" ");
        	}
        	System.out.print("| ");
        	for(j = 1;j <= 19;j++){
